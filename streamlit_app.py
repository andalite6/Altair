...
# Use fuzzy matching
similarity = difflib.SequenceMatcher(None, original_title_normalized,
crossref_title_normalized).ratio()
...
access_info = ''
# Citation
citation = f"{authors_str}. \"{title}.\" {container_title}{volume_issue}{pages},
{publication_year}{access_info}."
return citation.strip()
...
access_info = ''
# Citation
citation = f"{authors_str}. \"{title}.\" {container_title}{volume_issue}
({publication_year}){pages}{access_info}."
return citation.strip()
...
try:
response = requests.get(f"https://api.crossref.org/works?query={query}&rows=10",
timeout=10)
response.raise_for_status()
data = response.json()
...
return None
def generate_insight(user, category, prompt_text, response_text, knowledge_base, context,
...
value=0.7)
max_tokens = st.sidebar.number_input("Max Tokens:", min_value=50, max_value=2048,
value=150)
# Knowledge Base Input
st.subheader("Step 1: Provide Insight Report Writing Guidelines")
knowledge_base_input = st.text_area("Enter the guide on how to write an insight
report:", value=DEFAULT_KNOWLEDGE_BASE, height=200)
# Context Input
st.subheader("Step 2: Provide Up-to-Date Context")
context_input = st.text_area("Enter any additional context for the insight report:",
height=100)
# CSV Upload
st.subheader("Step 3: Upload Your CSV File")
sample_csv = "User,Category,Prompt,Response\nJohn Doe,Sales,\"How did sales
perform this quarter?\",\"Sales increased by 15%.\""
st.download_button(
"Download Sample CSV",
sample_csv,
"sample.csv",
...
logging.info(f"Generating insight for User: {row['User']}, Category:
{row['Category']}")
